...
1. Set the variables `certificate_signing_request` and `certificate_signing_request_private_key` as:
   - `certificate_signing_request`: base64-encoded certificate signing request in DER format.
   - `certificate_signing_request_private_key`: P256 certificate private key in PKCS8 format.

   For **non-production test-only** purposes, these can be generated with the commands below. Copy and replace the variable definitions generated by these scripts below into `.\prov_dev_client_ll_x509_csr_sample.c`.
   ...
   echo "static const char* certificate_signing_request_private_key ="; echo "$FORMATTED_CSR_KEY_PEM"

   echo "static const char* certificate_signing_request = \"$CSR_BASE64\";"
   ```
   ...
   echo "static const char* certificate_signing_request_private_key =`n$FORMATTED_CSR_KEY_PEM;";
   echo "static const char* certificate_signing_request = `"$CSR_BASE64`";"
   ```
